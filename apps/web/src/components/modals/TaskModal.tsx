...
export default function TaskModal({ open, onClose, onSuccess, task }: TaskModalProps) {
  ...
  const [form, setForm] = useState({
    ...
    progress: 0, taskStartDate: '', dueDate: '',
  });
  ...
  useEffect(() => {
    ...
    if (task) {
      setForm({
        ...
        departmentName: task.department?.name || task.intern?.department?.name || '',
      ...
      setForm({ title: '', description: '', internId: '', departmentId: '', departmentName: '', priority: 'Orta', status: 'Planlandı', progress: 0, taskStartDate: '', dueDate: '' });
    }
  }, [open, task]);
  ...
  const handleSubmit = async () => {
    ...
    if (form.taskStartDate && form.dueDate && form.taskStartDate > form.dueDate) {
    ...
    try {
      ...
      if (form.taskStartDate) payload.taskStartDate = form.taskStartDate;
    ...
    <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        ...
        <div className="modal-body">
          ...
          <div className="form-row">
            <div className="form-group">
              <label className="form-label">Görev Başlangıç Tarihi</label>
              <input
                className="form-input"
                type="date"
                value={form.taskStartDate}
                onChange={(e) => setForm((f) => ({ ...f, taskStartDate: e.target.value }))}
              />
            </div>
            <div className="form-group">
              <label className="form-label">Görev Bitiş Tarihi *</label>
              <input
                className="form-input"
                type="date"
                value={form.dueDate}
